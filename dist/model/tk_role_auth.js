...
=======
    static async addAuth(r_id = "", auth_id = ['']) {
        if (!r_id || Object.prototype.toString.call(auth_id) !== '[object Array]') {
            return false;
        };
        var roleId = await _model2.default.table("tk_role").field("id").where({ id: r_id }).findOne();
        if (!roleId || !roleId.id) {
            return false;
        };
        var authId = await _model2.default.table("tk_auth").field("id").where({ id: ["in", auth_id] }).select();
        var role_auth = await _model2.default.table("tk_role_auth").where({ r_id: r_id }).select();
        role_auth = role_auth.map(itme => itme.a_id);
        authId = authId.map(itme => itme.id);
        //找出新添加的role
        var addAuthId = [];
        authId.map(x => {
            if (role_auth.indexOf(x) == -1) {
                addAuthId.push({ r_id: r_id, a_id: x });
            }
        });

        // 找出要删除的auth
        var deleteAuthId = role_auth.filter(x => {
            if (authId.indexOf(x) == -1) return x;
        });
        if (addAuthId.length > 0) {
            var resAddRole = await _model2.default.table("tk_role_auth").add(addAuthId);
        }
        if (deleteAuthId.length > 0) {
            var reDelRole = await _model2.default.table("tk_role_auth").where({ r_id: r_id, a_id: ["in", deleteAuthId] }).delete();
        }
