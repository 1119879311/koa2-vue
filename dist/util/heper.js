...
exports.default = {
    ...
    isNull: val => {
        return undefined._proto_(val) == "[object Null]";
    },
    isFunction: val => {
        return undefined._proto_(val) == "[object Function]";
    },
    isUndefined(val) {
        return this._proto_(val) == "[object Undefined]";
    },
    isNumber: val => {
        return undefined._proto_(val) == "[object Number]";
    },
    isSymbol: val => {
        return undefined._proto_(val) == "[object Symbol]";
    },
    isBooleam: val => {
        return undefined._proto_(val) == "[object Booleam]";
    },
    isString: val => {
        return undefined._proto_(val) == "[object String]";
    },
    isObject(val) {
    ...
    objLen: val => {
        if (undefined.isObject(val)) return Object.keys(val).length;
        return 0;
    },
    splitobj: (obj = {}, live = "=") => {
        //对象键值拼接
    ...
    Aes: {
        ...
        aesEncrypt: function (val, key) {
            const cipher = _crypto2.default.createCipher('aes192', key);
        ...
        aesDecrypt: function (val, key) {
            const decipher = _crypto2.default.createDecipher('aes192', key);
    ...
    dataFormat: (date, format = "yyyy-MM-dd") => {
        //参数一:时间，参数，要显示的时间格式
    ...
    //递归对象， 实现一维数据结构
    recurOne: function (data = [], id = "id", pid = "pid", pidnum = 0, leve = 1) {
        //关联值 id and tid  = 0(定级)
    ...
    filterChrildId: function (data = [], id = "id", pid = "pid", idNum = 0, field = null) {
        //默认没有子集，返回空数组
        var _this = this;
